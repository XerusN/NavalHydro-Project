    ...
    for i in range(len(j)):
        ...
        while k < max_it and rel_diff > tol:
            ...
            for section in range(len(prop.geo.r_R)-1):
                aoa_ = aoa(beta[section], prop, section)
                cl[section] = cl_from_xfoil(aoa_, cl_a, section)
            cl[-1] = 0.     # Ensures proper gamma at the tip
            print('cl ', cl)
            ...
            gamma_new_2 = cl_2*v_inf_2*c_2/(2./prop.char.z)
            if k == 0:
                rel_diff = 1.
            else:
            ...
            beta_2 = np.arctan(ut_2/ua_2)
            beta_2[0] = np.arctan(v[i]/(2*pi*r_2[0]*n[i]))
            beta_2[-1] = np.arctan(v[i]/(2*pi*r_2[-1]*n[i]))
